...
void Splash (  )
{
    ...
    PG_Label l ( &splash2, PG_Rect ( 10, 10, 540, 320 ),
                 "I'm a splash screen" );
...
class MySliderLabel:public PG_Label,
      public PG_EventObject
{
  public:
    MySliderLabel ( PG_Widget * parent, const PG_Rect & r,
                    char *text ):PG_Label ( parent, r, text )
    {
    };
...
TestWindow::TestWindow ( PG_Widget * parent, const PG_Rect & r,
                         char *windowtext ):PG_Window ( parent, r, windowtext,
                                                        WF_SHOW_CLOSE |
                                                        WF_SHOW_MINIMIZE )
{
    ...
    PG_Button *b =
        new PG_Button ( NULL, BTN_ID_YES, PG_Rect ( 0, 0, 400, 50 ), "YES" );
    ...
    PG_Slider *s =
        new PG_Slider ( NULL, 20, PG_Rect ( 0, 0, 200, 20 ), PG_SB_HORIZONTAL );
    ...
    s->SetEventObject ( MSG_SLIDE, this,
                        ( MSG_CALLBACK_OBJ ) & TestWindow::
                        handler_slider_btntrans, b );

    WidgetList->AddWidget ( s );

    WidgetList->
        AddWidget ( new PG_LineEdit ( NULL, PG_Rect ( 0, 0, 80, 30 ) ) );

    PG_CheckButton *check =
        new PG_CheckButton ( NULL, 10, PG_Rect ( 0, 0, 200, 25 ),
                             "CheckButton 2" );
    ...
    PG_ScrollBar *scroll =
        new PG_ScrollBar ( this, 1, PG_Rect ( 415, 90, 20, 150 ) );

    scroll->SetRange ( 0, 100 );

    PG_ScrollBar *scroll1 =
        new PG_ScrollBar ( this, 2, PG_Rect ( 435, 90, 20, 150 ) );

    scroll1->SetRange ( 0, 255 );

    PG_DropDown *drop =
        new PG_DropDown ( this, 15, PG_Rect ( 260, 60, 200, 25 ) );
...
bool TestWindow::eventScrollPos ( int id, PG_Widget * widget,
                                  unsigned long data )
{
    if ( id == 1 )
    {
...
bool TestWindow::eventScrollTrack ( int id, PG_Widget * widget,
                                    unsigned long data )
{
    if ( id == 1 )
    {
...
{
    cout << "menu item '" << id << "' (\"" << item->
        getCaption (  ) << "\") clicked" << endl;
...
int testParagui ( int argc, char *argv[] )
{
    char theme[20];
    ...
    PG_LogMSG ( "screen initialized after %i ms",
                SDL_GetTicks (  ) - start_ticks );

    app.SetCursor ( app.GetTheme (  )->
                    FindSurface ( "Pointer", "Pointer", "normal" ) );
    ...
    TestWindow wnd ( NULL, PG_Rect ( 100, 100, 500, 300 ),
                     "My first Testwindow" );
    ...
    PG_LogMSG ( "creating 'wnd1' after %i ms",
                SDL_GetTicks (  ) - start_ticks );
    ...
    PG_RadioButton radio1 ( NULL, 1, PG_Rect ( 50, 0, 200, 25 ),
                            "RadioButton 1" );
    PG_RadioButton radio2 ( NULL, 2, PG_Rect ( 50, 25, 200, 25 ),
                            "RadioButton 2", &radio1 );
    PG_RadioButton radio3 ( NULL, 3, PG_Rect ( 50, 50, 200, 25 ),
                            "RadioButton 3", &radio1 );

    radio1.SetAlignment ( PG_TA_RIGHT );

    PG_RadioButton radio4 ( NULL, 4, PG_Rect ( 50, 90, 200, 25 ),
                            "RadioButton 4" );
    PG_RadioButton radio5 ( NULL, 5, PG_Rect ( 50, 115, 200, 25 ),
                            "RadioButton 5", &radio4 );
    PG_RadioButton radio6 ( NULL, 6, PG_Rect ( 50, 140, 200, 25 ),
                            "RadioButton 6", &radio4 );

    PG_CheckButton check1 ( NULL, 7, PG_Rect ( 300, 140, 180, 25 ),
                            "CheckButton 1" );
    ...
    subsubmenu.addMenuItem ( "Mordor", 1 ).addMenuItem ( "Minas Morgul",
                                                         2 ).
        addMenuItem ( "Orodruin", 3 );

    submenu.addMenuItem ( "Grendel", 1 ).addMenuItem ( "Beowulf",
                                                       2 ).
        addMenuItem ( "Hrothgar", 3 ).addMenuItem ( "Long ago", &subsubmenu );
    ...
    popmenu.addMenuItem ( "Tasty", 1, handle_menu_click ).
        ...
        addMenuItem ( "Saga",
                      &submenu ).addSeparator (  ).addMenuItem ( "Open Window",
                                                                 5,
                                                                 handle_menu_click,
                                                                 &wnd ).
    ...
    // create the slider
    PG_Slider slider ( NULL, 11, PG_Rect ( 50, 250, 300, 20 ),
                       PG_SB_HORIZONTAL );
    ...
    PG_Button quit ( NULL, BTN_ID_CLOSE, PG_Rect ( 600, 450, 100, 30 ),
                     "Quit" );
    ...
    show_wnd.SetEventObject ( MSG_BUTTONCLICK, &wnd,
                              ( MSG_CALLBACK_OBJ ) & TestWindow::
                              handle_show_window, ( void * ) &show_wnd );
