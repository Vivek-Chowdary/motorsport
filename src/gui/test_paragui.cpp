...
TestWindow::TestWindow (PG_Widget * parent, const PG_Rect & r, char *windowtext):PG_Window (parent, r, windowtext, WF_SHOW_CLOSE | WF_SHOW_MINIMIZE)
{
...
{
    cout << "menu item '" << id << "' (\"" << item->getCaption () << "\") clicked" << endl;
...
int testParagui (int argc, char *argv[])
{
    ...
    subsubmenu.addMenuItem ("Mordor", 1).addMenuItem ("Minas Morgul", 2).addMenuItem ("Orodruin", 3);

    submenu.addMenuItem ("Grendel", 1).addMenuItem ("Beowulf", 2).addMenuItem ("Hrothgar", 3).addMenuItem ("Long ago", &subsubmenu);
    ...
    popmenu.addMenuItem ("Tasty", 1, handle_menu_click).
        ...
        addMenuItem ("Saga", &submenu).addSeparator ().addMenuItem ("Open Window", 5, handle_menu_click, &wnd).addMenuItem ("Quit", 6, handle_menu_click, &app);
    ...
    if (tmp)
        PG_LogMSG ("sought for 'MyProgressBar' and found it (id '%d')", tmp->GetID ());
    tmp = wnd.FindChild (1001);
    if (tmp)
        PG_LogMSG ("sought for '1001' and found it (name '%s')", tmp->GetName ());
