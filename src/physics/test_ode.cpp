...
{
    return
        ( cmp ( _A ( 0, 0 ), 1.0 ) && cmp ( _A ( 0, 1 ), 0.0 )
          && cmp ( _A ( 0, 2 ), 0.0 ) && cmp ( _A ( 1, 0 ), 0.0 )
          && cmp ( _A ( 1, 1 ), 1.0 ) && cmp ( _A ( 1, 2 ), 0.0 )
          && cmp ( _A ( 2, 0 ), 0.0 ) && cmp ( _A ( 2, 1 ), 0.0 )
          && cmp ( _A ( 2, 2 ), 1.0 ) );
}
...
void testPad (  )
{
    ...
    printf ( "\t%s\n",
             strcmp ( s,
                      "0 1 4 4 4 8 8 8 8 12 12 12 12 16 16 16 16 " ) ? "FAILED"
             : "passed" );
}
...
void testMatrixMultiply (  )
{
    ...
    if ( C[0] != 116 || C[1] != 125 || C[2] != 134 || C[3] != 143 || C[4] != 224
         || C[5] != 242 || C[6] != 260 || C[7] != 278 )
        printf ( "\tFAILED (1)\n" );
    else
        printf ( "\tpassed (1)\n" );

    dMultiply1 ( C, A2, B, 2, 3, 4 );
    if ( C[0] != 160 || C[1] != 172 || C[2] != 184 || C[3] != 196 || C[4] != 196
         || C[5] != 211 || C[6] != 226 || C[7] != 241 )
        printf ( "\tFAILED (2)\n" );
    else
        printf ( "\tpassed (2)\n" );

    dMultiply2 ( C, A, B2, 2, 3, 4 );
    if ( C[0] != 83 || C[1] != 110 || C[2] != 137 || C[3] != 164 || C[4] != 164
         || C[5] != 218 || C[6] != 272 || C[7] != 326 )
        printf ( "\tFAILED (3)\n" );
    else
        printf ( "\tpassed (3)\n" );
}
...
void printMassParams ( dMass * m )
{
    ...
    printf ( "I    = [ %10.4f %10.4f %10.4f ]\n"
             "       [ %10.4f %10.4f %10.4f ]\n"
             "       [ %10.4f %10.4f %10.4f ]\n",
             m->_I ( 0, 0 ), m->_I ( 0, 1 ), m->_I ( 0, 2 ), m->_I ( 1, 0 ),
             m->_I ( 1, 1 ), m->_I ( 1, 2 ), m->_I ( 2, 0 ), m->_I ( 2, 1 ),
             m->_I ( 2, 2 ) );
}
...
void compareMassParams ( dMass * m1, dMass * m2, char *msg )
{
    ...
    if ( !
         ( cmp ( m1->mass, m2->mass ) && cmp ( m1->c[0], m2->c[0] )
...
void testMassFunctions (  )
{
    ...
    printf ( "\t" );
    dMassSetZero ( &m );
    TRAP_MESSAGE ( dMassSetParameters
                   ( &m, 10, 0.1, 0.2, 0.15, 3, 5, 14, 3.1, 3.2, 4 ),
                   printf ( " passed (2)\n" ), printf ( " FAILED (2)\n" ) );
    if ( m.mass == 10 && m.c[0] == REAL ( 0.1 ) && m.c[1] == REAL ( 0.2 )
         && m.c[2] == REAL ( 0.15 ) && m._I ( 0, 0 ) == 3 && m._I ( 1, 1 ) == 5
         && m._I ( 2, 2 ) == 14 && m._I ( 0, 1 ) == REAL ( 3.1 )
         && m._I ( 0, 2 ) == REAL ( 3.2 ) && m._I ( 1, 2 ) == 4
         && m._I ( 1, 0 ) == REAL ( 3.1 ) && m._I ( 2, 0 ) == REAL ( 3.2 )
         && m._I ( 2, 1 ) == 4 )
        printf ( "\tpassed (3)\n" );
    else
        printf ( "\tFAILED (3)\n" );
    ...
    if ( cmp ( m.mass, 3.73002719949386 ) && m.c[0] == 0 && m.c[1] == 0
    ...
    if ( cmp ( m.mass, 5.99961928996029 ) && m.c[0] == 0 && m.c[1] == 0
         && m.c[2] == 0 && cmp ( m._I ( 0, 0 ), 1.59461986077384 )
         && cmp ( m._I ( 1, 1 ), 4.57537403079093 )
         && cmp ( m._I ( 2, 2 ), 4.57537403079093 ) && m._I ( 0, 1 ) == 0
         && m._I ( 0, 2 ) == 0 && m._I ( 1, 2 ) == 0 && m._I ( 1, 0 ) == 0
         && m._I ( 2, 0 ) == 0 && m._I ( 2, 1 ) == 0 )
        printf ( "\tpassed (5)\n" );
    else
