...
int cmpIdentityMat3 (dMatrix3 A)
{
    return
        (cmp (_A (0, 0), 1.0) && cmp (_A (0, 1), 0.0) && cmp (_A (0, 2), 0.0) &&
         cmp (_A (1, 0), 0.0) && cmp (_A (1, 1), 1.0) && cmp (_A (1, 2), 0.0) && cmp (_A (2, 0), 0.0) && cmp (_A (2, 1), 0.0) && cmp (_A (2, 2), 1.0));
}
...
{
    HEADER;
    if (1e10 < dInfinity && -1e10 > -dInfinity && -dInfinity < dInfinity && !isnan (dInfinity) && !finite (dInfinity))
        printf ("\tpassed\n");
    else
        printf ("\tFAILED\n");
}
...
void testPad ()
{
    ...
    s[0] = 0;
    for (int i = 0; i <= 16; i++)
        sprintf (s + strlen (s), "%d ", dPAD (i));
    printf ("\t%s\n", strcmp (s, "0 1 4 4 4 8 8 8 8 12 12 12 12 16 16 16 16 ") ? "FAILED" : "passed");
}
...
void testNormalize3 ()
{
    ...
    for (i = 0; i < 1000; i++)
    {
        ...
        for (j = 0; j < 3; j++)
            n2[j] = n1[j];
        dNormalize3 (n2);
        if (dFabs (dDOT (n2, n2) - 1.0) > tol)
            bad |= 1;
        if (dFabs (n2[0] / n1[0] - n2[1] / n1[1]) > tol)
            bad |= 2;
        if (dFabs (n2[0] / n1[0] - n2[2] / n1[2]) > tol)
            bad |= 4;
        if (dFabs (n2[1] / n1[1] - n2[2] / n1[2]) > tol)
            bad |= 8;
        if (dFabs (dDOT (n2, n1) - dSqrt (dDOT (n1, n1))) > tol)
            bad |= 16;
        if (bad)
        {
...
void testPlaneSpace ()
{
    ...
    for (int i = 0; i < 1000; i++)
    {
        ...
        dNormalize3 (n);
        dPlaneSpace (n, p, q);
        if (fabs (dDOT (n, p)) > tol)
            bad = 1;
        if (fabs (dDOT (n, q)) > tol)
            bad = 1;
        if (fabs (dDOT (p, q)) > tol)
            bad = 1;
        if (fabs (dDOT (p, p) - 1) > tol)
            bad = 1;
        if (fabs (dDOT (q, q) - 1) > tol)
            bad = 1;
    }
    printf ("\t%s\n", bad ? "FAILED" : "passed");
}
...
void testMatrixMultiply ()
{
    ...
    if (C[0] != 116 || C[1] != 125 || C[2] != 134 || C[3] != 143 || C[4] != 224 || C[5] != 242 || C[6] != 260 || C[7] != 278)
        printf ("\tFAILED (1)\n");
    else
        printf ("\tpassed (1)\n");

    dMultiply1 (C, A2, B, 2, 3, 4);
    if (C[0] != 160 || C[1] != 172 || C[2] != 184 || C[3] != 196 || C[4] != 196 || C[5] != 211 || C[6] != 226 || C[7] != 241)
        printf ("\tFAILED (2)\n");
    else
        printf ("\tpassed (2)\n");

    dMultiply2 (C, A, B2, 2, 3, 4);
    if (C[0] != 83 || C[1] != 110 || C[2] != 137 || C[3] != 164 || C[4] != 164 || C[5] != 218 || C[6] != 272 || C[7] != 326)
        printf ("\tFAILED (3)\n");
    else
        printf ("\tpassed (3)\n");
}
...
void testSmallMatrixMultiply ()
{
    ...
    printf ("\t%s (1)\n", (dMaxDifference (a, a2, 3, 1) > tol) ? "FAILED" : "passed");
    ...
    printf ("\t%s (3)\n", (dMaxDifference (a, a2, 1, 3) > tol) ? "FAILED" : "passed");
...
void testInvertPDMatrix ()
{
    ...
    for (i = 0; i < MSIZE; i++)
    {
        ...
        {
            if (i != j)
                if (cmp (I[i * MSIZE4 + j], 0.0) == 0)
                    ok = 0;
        }
    }
    for (i = 0; i < MSIZE; i++)
    {
        if (cmp (I[i * MSIZE4 + i], 1.0) == 0)
            ok = 0;
    }
    if (ok)
        printf ("\tpassed (2)\n");
...
{
    int i, j;
    dReal A[MSIZE4 * MSIZE], L[MSIZE4 * MSIZE], DL[MSIZE4 * MSIZE], ATEST[MSIZE4 * MSIZE], d[MSIZE], diff;
    ...
    {
        for (j = 0; j < MSIZE; j++)
            DL[i * MSIZE4 + j] = L[i * MSIZE4 + j] / d[j];
    }
...
void testSolveLDLT ()
{
    dReal A[MSIZE4 * MSIZE], L[MSIZE4 * MSIZE], d[MSIZE], x[MSIZE], b[MSIZE], btest[MSIZE], diff;
...
{
    int i, j;
    dReal A[MSIZE4 * MSIZE], L[MSIZE4 * MSIZE], d[MSIZE], a[MSIZE], DL[MSIZE4 * MSIZE], ATEST[MSIZE4 * MSIZE], diff;
...
void testLDLTRemove ()
{
    ...
    dReal A[MSIZE4 * MSIZE], L[MSIZE4 * MSIZE], d[MSIZE],
        L2[MSIZE4 * MSIZE], d2[MSIZE], DL2[MSIZE4 * MSIZE], Atest1[MSIZE4 * MSIZE], Atest2[MSIZE4 * MSIZE], diff, maxdiff;
    HEADER;
        ...
        for (i = 0; i < MSIZE; i++)
        {
            for (j = 0; j < MSIZE; j++)
            {
                if (i != r && j != r)
                {
                    ...
                    if (jj >= r)
                        jj--;
                    if (A[i * MSIZE4 + j] != Atest1[ii * MSIZE4 + jj])
                        bad = 1;
                }
            }
        }
        if (bad)
            printf ("\trow/col removal FAILED for row %d\n", r);
...
void printMassParams (dMass * m)
{
    ...
    printf ("I    = [ %10.4f %10.4f %10.4f ]\n"
            "       [ %10.4f %10.4f %10.4f ]\n"
            "       [ %10.4f %10.4f %10.4f ]\n",
            m->_I (0, 0), m->_I (0, 1), m->_I (0, 2), m->_I (1, 0), m->_I (1, 1), m->_I (1, 2), m->_I (2, 0), m->_I (2, 1), m->_I (2, 2));
}
...
void compareMassParams (dMass * m1, dMass * m2, char *msg)
{
    ...
    if (!(cmp (m1->mass, m2->mass) && cmp (m1->c[0], m2->c[0]) && cmp (m1->c[1], m2->c[1]) && cmp (m1->c[2], m2->c[2])))
        ok = 0;
    for (i = 0; i < 3; i++)
        for (j = 0; j < 3; j++)
            if (cmp (m1->_I (i, j), m2->_I (i, j)) == 0)
                ok = 0;
    if (ok)
        printf ("\tpassed (%s)\n", msg);
    else
...
void testMassFunctions ()
{
    ...
    printf ("\t");
    dMassSetZero (&m);
    TRAP_MESSAGE (dMassSetParameters (&m, 10, 0, 0, 0, 1, 2, 3, 4, 5, 6), printf (" FAILED (1)\n"), printf (" passed (1)\n"));

    printf ("\t");
    dMassSetZero (&m);
    TRAP_MESSAGE (dMassSetParameters (&m, 10, 0.1, 0.2, 0.15, 3, 5, 14, 3.1, 3.2, 4), printf (" passed (2)\n"), printf (" FAILED (2)\n"));
    ...
    if (cmp (m.mass, 3.73002719949386) && m.c[0] == 0 && m.c[1] == 0 && m.c[2] == 0 &&
        ...
        m._I (0, 1) == 0 && m._I (0, 2) == 0 && m._I (1, 2) == 0 && m._I (1, 0) == 0 && m._I (2, 0) == 0 && m._I (2, 1) == 0)
        printf ("\tpassed (4)\n");
    else
        printf ("\tFAILED (4)\n");
